...
export class SkillRouter {
  ...
  readonly description =
    "Venus health-factor agent (inspect + negotiate + notify_funded) over A2A.";
    ...
    const result =
      envelope === null
        ? {
            ...
            skills: ["inspect", "negotiate", "notify_funded"],
          }
...
async function main(): Promise<void> {
  ...
  app.get("/ping", (_req, res) => {
    const busy = executor.isBusy() || isTickBusy();
    res.json({ status: busy ? "HEALTHY_BUSY" : "HEALTHY" });
  });
  ...
  app.get("/v1/position/:address", async (req, res) => {
    try {
      const health = await readAccountHealth(getAddress(req.params.address));
      res.json({ variant: loadStrategyConfig().variant, health });
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      res.status(400).json({ error: msg });
    }
  });
  app.get("/v1/tick", (_req, res) => {
    res.json({ last: lastTickSummary(), config: loadStrategyConfig() });
  });
