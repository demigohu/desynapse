...
import { readAccountHealth } from "./protocol/venus.js";
import { loadStrategyConfig } from "./strategy/config.js";
import { lastTickSummary } from "./strategy/tick.js";
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
  ...
  async dispatch(
    ...
    try {
      ...
      if (skill === "inspect") {
        return await inspectPosition(data);
      }
  ...
  execute = async (
    ...
    try {
      ...
      } else if (skill === "inspect") {
        result = await inspectPosition(data);
      } else {
...
async function inspectPosition(
  ...
  if (typeof raw !== "string") {
    return { error: 'inspect requires {"skill":"inspect","address":"0x..."}' };
  }
  const health = await readAccountHealth(getAddress(raw));
