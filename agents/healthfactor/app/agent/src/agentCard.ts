...
const INSPECT: AgentSkill = {
  id: "inspect",
  name: "Inspect a Venus health factor",
  description:
    'Send {"skill":"inspect","address":"0x..."} as a data part. Returns the live Venus Core Pool snapshot (HF, supply, borrow, per-market) plus this agent\'s variant threshold. Read-only; does not move funds.',
  tags: ["venus", "health-factor", "bnb-chain"],
  inputModes: ["application/json"],
  outputModes: ["application/json"],
};
...
export function buildAgentCard(
  ...
  return {
    name,
    description: `Venus health-factor guardian (${name}) — inspect positions, run a periodic protect loop, and sell ERC-8183 reports over A2A.`,
    ...
    skills:
      opts.commerceSkills === false
        ? [INSPECT]
        : [INSPECT, NEGOTIATE, NOTIFY_FUNDED],
    ...extra,
  };
}
