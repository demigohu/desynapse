...
export const LLM_READ_TOOLS: ToolSet = {
  ...
  venus_health_factor: tool({
    description:
      "Read-only Venus Core Pool health for an address on BSC testnet: HF, supply, borrow, per-market snapshot.",
    inputSchema: z.object({
      address: z
        .string()
        .describe("0x wallet to inspect (Altana smart account or EOA)"),
    }),
    execute: async ({ address }) => readAccountHealth(getAddress(address)),
  }),
