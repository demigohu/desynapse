# healthfactor — A2A seller agent (managed-platform trial)

The valuable Agent and the **SOLE key-holder/signer** for the healthfactor seller,
...
- `.env.local` — Agent secrets; on deploy they are sent to the **operator's**
  Secrets Manager (the scoped, consented commitment-#2 exception). Only the
